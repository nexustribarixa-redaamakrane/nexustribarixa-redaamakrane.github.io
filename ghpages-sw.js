 ...
 * 
 * HOW IT WORKS:
 *  - On first visit to your site, the SW is registered and caches error pages
 *    + essential assets (CSS, JS, SVGs) so they're available offline.
 *  - On subsequent visits, the SW intercepts every navigation request:
 *      → Network failure? Serve cached 500.html
 *      → 5xx response from server? Serve cached 500.html
 *      → Restricted path? Serve cached 403.html
 *      → Otherwise, pass through normally
 * 
 * CONFIGURATION:
 *  Edit RESTRICTED_PATHS below, or let the main library send config
 *  dynamically via postMessage (which persists in cache across SW restarts).
 * 
 * @version 1.0.0
 * @license MIT
 */
...
// Paths that should be blocked (serves 403.html instead).
// Supports exact paths and glob patterns:
//   '/admin'       — exact match
//   '/admin/*'     — matches one level deep (e.g. /admin/login)
//   '/admin/**'    — matches any depth (e.g. /admin/settings/users)
...
/* ══════════════════════════════════════════════
 *  CONFIG PERSISTENCE
 *  Stores dynamic config in the Cache API so it
...
async function getRestrictedPaths() {
  // Dynamic config takes priority over hardcoded
...
// INSTALL: Pre-cache error pages and essential assets
self.addEventListener('install', function(event) {
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(function(cache) {
        // Use addAll but don't let individual failures break installation
...
/* ══════════════════════════════════════════════
 *  FETCH INTERCEPTION — The Core Logic
 * ══════════════════════════════════════════════ */
...
self.addEventListener('fetch', function(event) {
  ...
  // Don't interfere with subresources (CSS, JS, images, API calls).
...
async function handleNavigation(request) {
  ...
  // ── Step 1: Check if path is restricted ──
  ...
  if (isPathRestricted(path, restricted)) {
    ...
    // Fallback if 403.html isn't cached
    return new Response(
  ...
  try {
    ...
    // If GitHub returns a 5xx error, serve our cached 500 page
    ...
    // If GitHub returns a real 403 (e.g. private repo), serve our 403
    ...
    // ── Step 3: Network failure — GitHub is DOWN ──
    // This is the money shot. The server is unreachable, but our
    // Service Worker is still running in the browser. Serve the
    // cached 500 page.
    ...
    // Ultimate fallback: even the cached page is gone
    return new Response(
...
/* ══════════════════════════════════════════════
 *  MESSAGE HANDLER
 *  Receives config updates from the main thread
 * ══════════════════════════════════════════════ */
...
self.addEventListener('message', function(event) {
  ...
  switch (event.data.type) {
    ...
    // Receive full configuration from main library
    case 'SET_CONFIG':
      saveConfig(event.data.config || {});
      break;

    // Force re-cache of error pages and assets
    case 'UPDATE_CACHE':
      event.waitUntil(
    ...
    // Health check — respond so main thread knows SW is alive
    case 'PING':
